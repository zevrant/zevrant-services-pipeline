node("master") {
    ...
    stage("Build Artifact") {
        ...
        String json = readJSON text: (sh(returnStdout: true, script: "aws ssm get-parameter --name /android/signing/keystore"))
        keystore = json['SecretString']
        writeFile('./zevrant-services.txt', keystore)
        sh "base64 -d ./zevrant-services.txt > ./zevrant-services.p12"
        json = readJSON text: (sh(returnStdout: true, script: "aws ssm get-parameter --name /android/signing/keystore"))
        password = json['SecretString']
        sh "set +x; SIGNING_KEYSTORE='./zevrant-services.p12 KEYSOTRE_PASSWORD=$password bash gradlew clean assemble$variant --no-daemon"
    }
