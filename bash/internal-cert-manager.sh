...
sed -i "s/\${SERVICE_NAME}/${1}.zevrant-services.internal/g" ~/openssl.conf
...
certificateRequest="{\"certificateRequest\":\"$certificateRequest\",\"ip\":\"${1}.zevrant-services.internal}\"}"
curl --insecure https://zevrant-01.zevrant-services.internal:9009/zevrant-certificate-service/certs --data "$certificateRequest" --user "$username":"$password" -H "Content-Type: application/json" -X POST > /root/public.crt
today="$(date --utc --iso-8601)"
mv /root/private.pem "/etc/pki/archive/${1}.zevrant-services.internal-${today}.key"
mv /root/public.crt "/etc/pki/archive/${1}.zevrant-services.internal-${today}.crt"
ln -s "/etc/pki/archive/${1}.zevrant-services.internal-${today}.crt" "/etc/pki/archive/${1}.zevrant-services.com-${today}.key" /etc/pki/live
