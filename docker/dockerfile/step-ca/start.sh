...
if [[ ! -d "/opt/step-ca/.step/authorities/${CA_NAME}" ]]; then
  step ca init --authority="$CA_NAME" --dns=certificate-authority --deployment-type=standalone -name "$CA_NAME" --address='0.0.0.0:8443' --provisioner="$CA_NAME" --password-file="$PASSWORD_FILE_PATH"
